...
double model_mpi(vector<double> &t_rel, vector<double> &flux,
                 vector<double> &weights, double period, double duration,
                 double phase, int rank, int size) {
  ...
  for (size_t i = 0; i < local_t_rel.size(); ++i) {
    local_is_transit[i] = ((fmod(local_t_rel[i], period) >= phase) &&
                           fmod(local_t_rel[i], period) <= phase + duration)
                              ? 1
                              : 0;
  }

  // Compute local r, s, and wx
  double local_r = inner_product(local_weights.begin(), local_weights.end(),
                                 local_is_transit.begin(), 0.0);
  double local_s = 0.0;
  for (size_t i = 0; i < local_flux.size(); ++i) {
    local_s += local_weights[i] * local_flux[i] * local_is_transit[i];
  }
  double local_wx = inner_product(local_weights.begin(), local_weights.end(),
                                  local_flux.begin(), 0.0, plus<double>(),
                                  [](double w, double x) { return w * x * x; });
  ...
  ss << "  local_is_transit[" << 0 << "] = " << local_is_transit[0] << endl
     << "  local_is_transit[" << local_is_transit.size()
     << "] = " << local_is_transit[local_is_transit.size() - 1] << endl
     << "  next: ";
  ...
  ss << endl;
  ss << "    local_r: " << local_r << ", local_s: " << local_s
     << ", local_wx: " << local_wx << endl;
  cout << ss.str();
  // ss << "";
...
BLSResult bls_mpi(vector<double> &time, vector<double> &flux,
  ...
  MPI_Op_create((MPI_User_function *)minloc_reduction, true,
                &MPI_BLSResult_minloc);

  BLSResult global_result;
  MPI_Reduce(&local_result, &global_result, 1, MPI_BLSResult,
             MPI_BLSResult_minloc, 0, MPI_COMM_WORLD);
