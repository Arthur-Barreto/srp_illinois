...
vector<double> auto_period_omp(double minimum_period, double maximum_period,
...
vector<SPECParameters> spec_generator_omp(vector<double> &time) {
  ...
  vector<double> periods =
      auto_period_omp(p_params.minimum_period, p_params.maximum_period,
...
double model_omp(vector<double> &t_rel, vector<double> &flux,
...
BLSResult bls_omp(vector<double> &time, vector<double> &flux,
...
#pragma omp parallel for
  for (size_t i = 0; i < s_params.size(); ++i) {
    ...
    double d_value =
        model_omp(t_rel, normalized_flux, weights, period, duration, phase);
