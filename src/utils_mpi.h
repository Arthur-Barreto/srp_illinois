...
double model_mpi(const std::vector<double> &t_rel,
                 const std::vector<double> &flux,
                 const std::vector<double> &weights, double period,
                 double duration, double phase, int rank, int size);

void minloc_reduction(void *in, void *inout, int *len, MPI_Datatype *dptr);

BLSResult bls_mpi(std::vector<double> &time, std::vector<double> &flux,
                  std::vector<double> &flux_err,
                  std::vector<SPECParameters> &s_params, int rank, int size);
