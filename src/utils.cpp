...
vector<double> logarithmic_space_omp(double start, double end, size_t num) {
    vector<double> result(num);
    double step = (log10(end) - log10(start)) / (num - 1);
#pragma omp parallel for
    for (size_t i = 0; i < num; ++i) {
...
vector<double> compute_trel_omp(vector<double> &time) {
    double t0 = min_value(time);
    vector<double> t_rel(time.size(), 0.0);
...
vector<double> normalize_omp(vector<double> &flux) {
    vector<double> normalized_flux(flux.size());
...
#pragma omp parallel for reduction(+ : squared_sum)
    for (size_t i = 0; i < flux.size(); ++i) {
        squared_sum += normalized_flux[i] * normalized_flux[i];
    }
...
vector<double> compute_weights_omp(vector<double> &flux_err) {
    vector<double> weights(flux_err.size());
#pragma omp parallel for
    for (size_t i = 0; i < flux_err.size(); ++i) {
        weights[i] = 1.0 / (flux_err[i] * flux_err[i]);
